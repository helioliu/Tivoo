...
public class SortedListOutputter extends AbstractHtmlOutputter{
	...
	protected String appendFormatting(List<Event> events, DateTime dt, Body body){ 
        ...
        B b = new B();
        b.appendChild(new Text(dt.toString("MM/dd/YYYY")+"<br/>"));
        p.appendChild(b);
	...
	private void checkNewDay(P p, DateTime first, DateTime second){
		if(!isSameDate(first, second)){
			first = new DateTime(second);
			B b = new B();
			b.appendChild(new Text(first.toString("MM/dd/YYYY")+"<br/>"));
			p.appendChild(b);
		}
